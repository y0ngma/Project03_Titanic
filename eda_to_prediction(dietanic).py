...
Original file is located at
    https://colab.research.google.com/drive/1lBwNL0GVjZofCvsSapkl4BOY4xjkUZTS
"""
...
# predict_proba는 [0일확률, 1일 확률] 반환
model.predict_proba(test_X)[:,:][:3]

test_Y[:9].T

# predict()는 0, 1을 default threshold에 따라 반환
# 이 값을 정답과 같게 하는 threshold 값을 찾아보자
model.predict(test_X)[:9]

model.predict_proba(test_X)[:,1][:9]

# threshold값을 조정해보자
model.predict_proba(test_X)[:,1][:9] > 0.5

(model.predict_proba(test_X)[:,0][:9] > 0.5).astype(int)

# 0.1로 잡으면 다 생존한다고 하는 안좋은 모델이 된다
( model.predict_proba(test_X)[:, 1][:10] > 0.1 ).astype(int)

# 그래서 적당히 바꿔보면서 추이를 알아보라
threshold = 0.2
prediction1 = ( model.predict_proba(test_X)[:, 1] > threshold ).astype(int)
print( 'Accuracy for rbf SVM is ', metrics.accuracy_score(prediction1,test_Y) )

# 그 작업을 반복문으로 실행
for temp_thres in np.arange(0, 1, 0.1):
  threshold = temp_thres
  prediction1 = ( model.predict_proba(test_X)[:, 1] > threshold ).astype(int)
  print( 'Accuracy for rbf SVM is  {:.2f}%'.format(
      100 * metrics.accuracy_score(prediction1, test_Y)) )
...
for temp_thres in np.arange(0, 1, 0.01):
  ...
  prediction1 = (model.predict_proba(test_X)[:, 1] > threshold).astype(int)
  if temp_score >= 83:
    print( 'Accuracy for rbf SVM is {:.2f}% by{:>6.2f}'.format(temp_score, temp_thres) )
...
## 100줄 출력말고 정확도높은거 10개정도만 출력해보려다 81% 같은값만 나옴
## 결국 해당 threshold를 출력해야해서 난감..
# 리스트를 어레이로 만들기
tmp = np.array(score_array)
# array에서 가장 큰값의 인덱스를 반환하기
i = np.argsort(tmp)
print('정확도는{:>6.2f} 에서{:>6.2f}%까지'.format(tmp[i][0], tmp[i][-1]))

# 한줄로 만들기
sorted_score = np.array(score_array)[ np.argsort(np.array(score_array)) ]
sorted_score[0], sorted_score[-1]

# np.argsort()인자는 array일 필요 없는듯
tmp = np.array(score_array)[np.argsort(score_array)]
tmp[0], tmp[-1]

"""#### 사분위값 구하기
- 정확도 상위10%의 값, threshold에 관한 고찰
  - 100명회사 99직원평균 100만원월급, 사장 1억월급일때  
  평균보다 중앙값이 대표값으로 적절
  1. top 10%의 값 계산
    - np.quantile(data, 0.9)
  1. top 10% 값 이상일때의 정확도와, threshold 출력
"""

# 2분위수는 중앙값이다
num = [0, 110, 120, 130, 140]
np.mean(num), np.median(num), np.quantile(num, 0.5), np.quantile(num, 0.75)

# 중앙값을 가지는 인덱스 찾는법 
list( filter(lambda x: score_array[x]==np.quantile(score_array, 0.5), range(len(score_array))) )
[ i for i in range(len(score_array)) if score_array[i]==np.median(score_array) ]
[ key for key, value in enumerate(score_array) if value==np.median(score_array)]

# 중앙값을 가지는 인덱스 찾는법
np.quantile(score_array, 0.5), list(filter(lambda x: score_array[x]==np.quantile(score_array, 0.5), range(len(score_array))) )

# 평균을 가지는 인덱스 찾기
np.mean(score_array), [ i for i in range(len(score_array)) if score_array[i]==np.mean(score_array) ]

# 최대값을 가지는 인덱스 찾기
np.quantile(score_array, 1), [ key for key, value in enumerate(score_array) if value==max(score_array) ]

# 정확도 리스트에서 3분위값 이상의 threshold 출력
print( '3분위값 이상의 threshold {}'.format(np.argsort(score_array)[75:]) )
...
model.fit(train_X,train_Y)
prediction3=model.predict(test_X)
print('Default threshold accuracy = {:>8.3f} : Default threshold '
.format(100 * metrics.accuracy_score(prediction3,test_Y)) )
...
print(' 10th accuracy : threshold = {:>8.3f} : {:>4}\n Best accuracy : threshold = {:>8.3f} : {:>4}'
.format( np.quantile(score_array, 0.9), 0.01 * np.argsort(score_array)[-10], 
                      max(score_array), 0.01 * np.argsort(score_array)[-1] ))
...
prediction4=model.predict(test_X)
print('Default threshold accuracy = {:>8.3f} : Default threshold '
.format(100 * metrics.accuracy_score(prediction4,test_Y)) )
...
prediction5=model.predict(test_X)
print('Default threshold accuracy = {:>8.3f} : Default threshold '
.format(100 * metrics.accuracy_score(prediction5,test_Y)) )
...
print(' 10th accuracy : threshold = {:>8.3f} : {:>4.2f}\n Best accuracy : threshold = {:>8.3f} : {:>4.2f}'
.format( np.quantile(score_array, 0.9), 0.01 * np.argsort(score_array)[-10], 
                      max(score_array), 0.01 * np.argsort(score_array)[-1] ))
